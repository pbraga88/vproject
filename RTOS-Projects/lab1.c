...
void Task3(void){
  ...
  if((current == 0) && (prev1 != 0)){
    ...
    //BSP_Buzzer_Set(512);  *****COMMENTED*****         // beep until next call of this task
  }
  prev1 = current;
...
int main(void){
  ...
  TExaS_Init(GRADER, 2833 );         // initialize the Lab 1 grader
//  TExaS_Init(LOGICANALYZER, 1000);  // initialize the Lab 1 logic analyzer
  ...
  while(1){
		int count = 0;
    for(int i=0; i<1000; i++){ // runs at about 1MHz after modification
			Task0();  // sample microphone
      if(count>=99){  // runs at about 10Hz after modification
